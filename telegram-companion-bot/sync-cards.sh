...
for entry in "telegram-bot:nora:nora.json" "bonnie-bot:bonnie:bonnie.json" "cass-bot:cass:cass.json" "emily-bot:emily:emily_harper.json" "priya-bot:priya:priya.json" "jules-bot:jules:jules_nakagawa.json"; do
  dir="$HOME/${entry%%:*}"
  rest="${entry#*:}"
  name="${rest%%:*}"
  default_card="${rest#*:}"
