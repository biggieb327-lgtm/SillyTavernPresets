...
3. **Match against the known signature table before inventing a new theory:**
   ...
   | `ModuleNotFoundError` crash-loop | selected immutable dependency layer is absent/damaged, or the lock omitted a real import | `journalctl` names the module; run `vps-sync.sh --rollback <instance>`, then fix `requirements.txt` + regenerate `requirements.lock` on main — never mutate a selected venv |
