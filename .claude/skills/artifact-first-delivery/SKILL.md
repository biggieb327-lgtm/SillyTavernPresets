...
4. In this repo, work isn't delivered until pushed — deploys pull from `main` on the VPS via `vps-sync.sh` (reading the whole locked release from the checkout, not curl-piped raw files), and the session container is ephemeral.
