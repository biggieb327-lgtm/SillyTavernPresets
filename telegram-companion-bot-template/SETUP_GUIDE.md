...
2. Send it `/newbot`.
3. Pick a name for your bot (e.g. "Priya") — this is the display name.
4. Pick a username ending in `bot` (e.g. `priya_companion_bot`) — this must be unique.
...
```bash
cd ~
git clone https://github.com/biggieb327-lgtm/telegram-companion-bot.git
cp -r telegram-companion-bot ~/telegram-bot
cd ~/telegram-bot
```

(You can delete the `telegram-companion-bot` folder afterward if you want — only
...
```
WEATHER_LOCATION=Portland
WEATHER_LAT=45.5152
WEATHER_LON=-122.6784
...
The bot comes with an example character, **Priya** (`priya.json`). You can:

- **Use her as-is** — just to get things working, then customize later.
- **Edit her** — `priya.json` is a SillyTavern v2 character card (JSON). You can
...
`priya_base.png` (or whatever `SELFIE_BASE` is set to in `.env`) in this folder.
...
🚀 Priya is running... (home: /data/data/com.termux/files/home/telegram-bot)
```
...
Want a second bot with a different personality? See "Running a second character" in
