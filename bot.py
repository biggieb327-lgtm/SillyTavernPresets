#!/usr/bin/env python3
"""Telegram <-> NanoGPT (GLM-4.6-turbo:thinking) bot.

Single file, long-polling, SQLite-backed persistence. Designed to run under
Termux on Android. Secrets come from the environment (BOT_TOKEN,
NANOGPT_API_KEY); model calls go through NanoGPT's OpenAI-compatible endpoint
using httpx (no openai SDK on purpose).
"""
...
DB_PATH = os.path.join(os.path.expanduser("~"), "bot.db")
...
# httpx logs every request at INFO; keep it quiet.
...
# --------------------------------------------------------------------------- #
# Persistence (SQLite at ~/bot.db)
# --------------------------------------------------------------------------- #

# One connection shared across the asyncio loop and the job queue. sqlite3 is
# synchronous; a lock keeps concurrent access safe.
...
def add_message(chat_id: int, role: str, content: str) -> None:
    ...
    with _db_lock:
        ...
        # Keep at most MAX_TURNS*2 rows (user+assistant per turn) for this chat.
        _db.execute(
            """
...
def strip_thinking(text: str) -> str:
    ...
    # Stray opening tag with no close: drop it too.
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "Hi! I'm wired up to GLM-4.6. Just send me a message and I'll reply.\n"
...
async def post_init(application) -> None:
    log.info("Bot started. Known chats: %d", len(known_chats()))
